...
namespace ConsoleApp1
{
    ...
    {
        public static PdfDocument ConvertTMLtoPDF(string filePathTML, PlanSetup plan /*PlanViewModel plan*/)
        {
            ...
            string fontFamily = fontCollection.Families.First().Name;
            MessageBox.Show(fontFamily);
            //XFont font = new XFont(fontFamily, fontSize, GetFontStyle(fontWeight));
            XFont font = new XFont(fontFamily, 2);
            string text = "";
            int lineSpacing = 1;
            ...
            try
            {
                foreach (string line in TMLSheet)
                {
                    ...
                    if (line.StartsWith("PaperOrientation"))
                    {
                        ...
                        {
                            firstPage.Orientation = PdfSharpCore.PageOrientation.Landscape;
                    ...
                    if (line.StartsWith("PaperSize"))
                    {
                        ...
                        {
                            firstPage.Size = PdfSharpCore.PageSize.A3;
                        }
                        if (line.Split('=')[1].Contains("Legal"))
                        {
                            firstPage.Size = PdfSharpCore.PageSize.Legal;
                        }
                        if (line.Split('=')[1].Contains("Letter"))
                        {
                            firstPage.Size = PdfSharpCore.PageSize.Letter;
                        }
                        if (line.Split('=')[1].Contains("Tabloid"))
                        {
                            firstPage.Size = PdfSharpCore.PageSize.Tabloid;
                        }
                    }
